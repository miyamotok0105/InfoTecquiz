...
typedef NS_ENUM(NSInteger, FUIAlertQuizViewStyle) {
    FUIAlertQuizViewStyleDefault = 0,
    FUIAlertQuizViewStyleSecureTextInput,
    FUIAlertQuizViewStylePlainTextInput,
    FUIAlertQuizViewStyleLoginAndPasswordInput
};

@protocol FUIAlertQuizViewDelegate;

@interface FUIAlertQuizView : UIView

- (id)initWithTitle:(NSString *)title
            message:(NSString *)message
           delegate:(id<FUIAlertQuizViewDelegate>)delegate
  cancelButtonTitle:(NSString *)cancelButtonTitle
  otherButtonTitles:(NSString *)otherButtonTitles, ... NS_REQUIRES_NIL_TERMINATION;

- (id)initWithTitleTest:(NSString *)title
            message:(NSString *)message
           delegate:(id<FUIAlertQuizViewDelegate>)delegate
  cancelButtonTitle:(NSString *)cancelButtonTitle
  otherButtonTitles:(NSString *)otherButtonTitles, ... NS_REQUIRES_NIL_TERMINATION;
...
- (id)initWithTitle4:(NSString *)title
            message:(NSString *)message
           delegate:(id<FUIAlertQuizViewDelegate>)delegate
  cancelButtonTitle:(NSString *)cancelButtonTitle
  otherButtonTitles1:(NSString *)otherButtonTitles1
  otherButtonTitles2:(NSString *)otherButtonTitles2
  otherButtonTitles3:(NSString *)otherButtonTitles3
  otherButtonTitles4:(NSString *)otherButtonTitles4
, ... NS_REQUIRES_NIL_TERMINATION;

@property(nonatomic,assign) id<FUIAlertQuizViewDelegate> delegate;    // weak reference
...
// Flat Alert view style - defaults to FUIAlertQuizViewStyleDefault
@property(nonatomic,assign) FUIAlertQuizViewStyle alertViewStyle;
...
@protocol FUIAlertQuizViewDelegate <NSObject>
@optional

// Called when a button is clicked. The view will be automatically dismissed after this call returns
- (void)alertView:(FUIAlertQuizView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex;

// Called when we cancel a view (eg. the user clicks the Home button). This is not called when the user clicks the cancel button.
// If not defined in the delegate, we simulate a click in the cancel button
// TODO: not implemented
//- (void)alertViewCancel:(FUIAlertQuizView *)alertView;

- (void)willPresentAlertView:(FUIAlertQuizView *)alertView;  // before animation and showing view
- (void)didPresentAlertView:(FUIAlertQuizView *)alertView;  // after animation

- (void)alertView:(FUIAlertQuizView *)alertView willDismissWithButtonIndex:(NSInteger)buttonIndex; // before animation and hiding view
- (void)alertView:(FUIAlertQuizView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex;  // after animation
