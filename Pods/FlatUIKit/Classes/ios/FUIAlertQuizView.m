...
@implementation FUIAlertQuizView

+ (void)initialize {
    if (self == [FUIAlertQuizView class]) {
        FUIAlertQuizView *appearance = [self appearance];
...
- (id)initWithTitle4:(NSString *)title
  ...
  otherButtonTitles4:(NSString *)otherButtonTitles4, ... {
    ...
    if (self) {
        ...
        va_list args;
        va_start(args, otherButtonTitles1);
        for (NSString *arg = otherButtonTitles1; arg != nil; arg = va_arg(args, NSString*)) {
            [self addButtonWithTitle:arg];
        }
        va_end(args);
...
//        }
        ...
        va_start(args, otherButtonTitles2);
        for (NSString *arg = otherButtonTitles2; arg != nil; arg = va_arg(args, NSString*)) {
            [self addButtonWithTitle:arg];
        }
        va_end(args);
        
        va_start(args, otherButtonTitles3);
        for (NSString *arg = otherButtonTitles3; arg != nil; arg = va_arg(args, NSString*)) {
            [self addButtonWithTitle:arg];
        }
        va_end(args);
        
        va_start(args, otherButtonTitles4);
        for (NSString *arg = otherButtonTitles4; arg != nil; arg = va_arg(args, NSString*)) {
            [self addButtonWithTitle:arg];
        }
        va_end(args);
    }
...
//        self.backgroundOverlay.frame = CGRectMake(100, 250, self.bounds.size.width, self.bounds.size.height);
        
        CGFloat padding = 15;
//        CGRect contentContainerFrame = CGRectMake(padding, padding + 100, 0, 0);
        ...
        if (self.alertViewStyle == FUIAlertQuizViewStylePlainTextInput || self.alertViewStyle == FUIAlertQuizViewStyleSecureTextInput) {
            if (self.alertViewStyle == FUIAlertQuizViewStyleSecureTextInput) {
        ...
        if (self.alertViewStyle == FUIAlertQuizViewStyleLoginAndPasswordInput) {
            [self.textFields[0] setFrame:(CGRect){{0, self.messageLabel.frame.origin.y + self.messageLabel.frame.size.height + 10},{self.alertContentContainer.frame.size.width, 40}}];
...
- (CGSize) calculateSize {
    ...
    if (self.alertViewStyle == FUIAlertQuizViewStyleDefault) {
        textFieldHeight = -10;
    }
    if (self.alertViewStyle == FUIAlertQuizViewStylePlainTextInput || self.alertViewStyle == FUIAlertQuizViewStyleSecureTextInput) {
